...
class Agent:
    ...
    async def execute(self, message_history):
        ...
        stream = await self.client.chat.completions.create(messages=copied_message_history, stream=True,
                                                           tools=self.tools, tool_choice="auto", **self.gen_kwargs)
        ...
        async for part in stream:
            if part.choices[0].delta.tool_calls:
                ...
                # Make sure the lists are long enough to accommodate the index
                while len(function_names) <= tool_call.index:
                    function_names.append("")  # Append empty string if needed

                while len(argumentss) <= tool_call.index:
                    argumentss.append("")  # Append empty string if needed

                function_names[tool_call.index] += function_name_delta
                argumentss[tool_call.index] += arguments_delta

            if token := part.choices[0].delta.content or "":
                await response_message.stream_token(token)

        for function_name, arguments in zip(function_names, argumentss):
            ...
            if function_name == "updateArtifact":
                ...
                if filename and contents:
                    ...
                    stream = await self.client.chat.completions.create(messages=message_history, stream=True,
                                                                       **self.gen_kwargs)
                    async for part in stream:
                        if token := part.choices[0].delta.content or "":
                            await response_message.stream_token(token)
